...
public class Whl extends Token {

    private final Token _op;
    private final Expression _pred;

    public Whl(final Token op, final Expression pred, final Encoding enc) {
        super(enc);
    ...
    protected ParseResult parseImpl(final String scope, final Environment env, final Encoding enc) throws IOException {
        final boolean predicate = _pred.eval(env, enc);
        if (!predicate) { return new ParseResult(true, env); }
